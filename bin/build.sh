...
# West Build (left)
west build -s zmk/app ${PRISTINE} -d build/left -b adv360_left -S studio-rpc-usb-uart -- -DZMK_CONFIG="${PWD}/config" -DCONFIG_ZMK_STUDIO=y
...
if [ "${BUILD_RIGHT}" = true ]; then
    # West Build (right)
    west build -s zmk/app ${PRISTINE} -d build/right -b adv360_right -- -DZMK_CONFIG="${PWD}/config"
